...
function ListingCard({ item }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[1.5rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col md:flex-row group">
      ...
      <div className="flex-1 p-6 flex flex-col justify-between overflow-hidden"><div><h3 className="text-xl font-black text-dark leading-tight line-clamp-1">{item.title}</h3><p className="text-[13px] text-gray-400 font-medium mt-1 uppercase tracking-wider truncate mb-4">{item.location || 'Central Delhi'}</p><div className="flex items-center flex-wrap gap-x-4 gap-y-1 text-[12px] text-gray-500 font-medium mb-4 whitespace-nowrap"><div className="flex items-center gap-1.5 shrink-0"><MapPin size={14} className="text-gray-400" /> <span>1.2 mi fr <span className="text-gray-800 font-black">City Cer</span></span></div><div className="flex items-center gap-2 opacity-80 text-gray-400 shrink-0">( <Bus size={13} /> 11m · <Building2 size={13} /> 21m · <Footprints size={13} /> 24m )</div></div><hr className="border-t border-dashed border-gray-200 my-4" /><div className="space-y-1"><div className="grid grid-cols-2 gap-1.5"><div className="px-2 py-1.5 bg-[#EBF2FF] text-[#2563EB] text-[9.5px] font-bold rounded-full flex items-center gap-1 border border-dashed border-[#2563EB] overflow-hidden"><div className="bg-[#2563EB] text-white p-0.5 rounded-sm shrink-0"><Zap size={9} fill="currentColor" /></div> <span className="flex-1 text-center truncate">{item.offers || '2 Offers'}</span></div><div className="px-2 py-1.5 bg-[#F0FDF4] text-[#166534] text-[9.5px] font-bold rounded-full flex items-center gap-1 border border-[#DCFCE7] overflow-hidden"><div className="bg-[#166534] text-white p-0.5 rounded-full shrink-0"><Zap size={9} /></div> <span className="flex-1 text-center truncate">Instant Booking</span></div></div><div className="grid grid-cols-2 gap-1.5"><div className="px-2 py-1.5 bg-[#FDF2E9] text-[#78350F] text-[9.5px] font-bold rounded-full flex items-center gap-1 border border-transparent overflow-hidden"><CalendarDays size={12} className="shrink-0" /> <span className="flex-1 text-center truncate">Pay In Instalment</span></div><div className="px-2 py-1.5 bg-[#FDF2E9] text-[#78350F] text-[9.5px] font-bold rounded-full flex items-center gap-1 border border-transparent overflow-hidden"><ShieldCheck size={12} className="shrink-0" /> <span className="flex-1 text-center truncate">No Visa No Pay</span></div></div><div className="grid grid-cols-2 gap-1">{(item.amenities || ['Laundry Room', 'Study Room', 'Modern Gym', 'WiFi']).slice(0, 4).map((tag, idx) => <span key={idx} className="px-3 py-1.5 bg-white text-gray-600 text-[10px] font-bold rounded-full flex items-center gap-2 border border-gray-200 truncate"><LayoutGrid size={13} className="flex-shrink-0" /> {tag.replace(/-/g, ' ')}</span>)}</div></div></div></div>
